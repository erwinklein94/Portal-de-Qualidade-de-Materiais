-- Certificados de inspeção da equipe de qualidade dos fornecedores de Subcomponentes.
...
values ('subcomponent-certificates', 'subcomponent-certificates', false, 20971520, array['application/pdf'])
on conflict (id) do update set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
  allowed_mime_types = excluded.allowed_mime_types;

create policy "subcomponent suppliers upload own certificates"
on storage.objects for insert to authenticated
with check (
  bucket_id = 'subcomponent-certificates'
  and (storage.foldername(name))[1] = (select private.current_supplier_id())::text
  and (select private.current_area_id()) = (select id from public.material_areas where code = 'subcomponents')
);

create policy "team or owner read subcomponent certificates"
...
create policy "subcomponent suppliers delete own failed uploads"
...
where area_id = (select id from public.material_areas where code = 'subcomponents')
